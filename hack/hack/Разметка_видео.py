...
def process_video(uploaded_file):
    ...
    st.header("Музыка.")
    mask = st.session_state.whisper_data["text"].str.contains("МУЗЫКА", case=False, na=False)
    music_df = st.session_state.whisper_data[mask]

    if music_df.shape[0]:
        st.write("Строки, содержащие музыку:")
        for i in range(music_df.shape[0]):
            music_text = music_df.iloc[i]["text"]
            start = music_df.iloc[i]["start"]
            end = music_df.iloc[i]["end"]
            st.write(f'{music_text} c {int(start // 60):02}:{int(start % 60):02} по {int(end // 60):02}:{int(end % 60):02}')
            # st.write(music_df.iloc[i]["text"], music_df.iloc[i]["start"], music_df.iloc[i]["end"])
    else:
